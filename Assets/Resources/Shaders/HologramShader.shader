Shader "FX/Hologram Shader"
{
	Properties
	{
		...
		_GlitchIntensity ("Glitch Intensity", Range(0.0, 0.5)) = 0
	}
		...
		Pass
		{
			...
			CGPROGRAM
				...
				v2f vertexFunc(appdata IN){
					...
					//Glitch
					IN.vertex.x += sin(_Time.x * _GlitchSpeed * 5 * IN.vertex.z) * _GlitchIntensity;
					IN.vertex.y += sin(_Time.y * _GlitchSpeed * 5 * IN.vertex.x) * _GlitchIntensity;
					IN.vertex.z += sin(_Time.z * _GlitchSpeed * 5 * IN.vertex.y) * _GlitchIntensity;
